...
const verifyToken = asyncHandler(async (req, res, next) => {
  try {
    ...
    const userinfo =
      (await Users.findById(decoded._id).select("-password")) ||
      (await SuperUser.findById(decoded._id).select("-password")) ||
      (await OnlineUser.findById(decoded._id).select("-password"));
